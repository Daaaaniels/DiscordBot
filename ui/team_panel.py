...
async def send_panel(interaction):
    try:
        print("\U0001f9ea Sending team panel...")
...
class LeaveTeamButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        changed = await remove_user_from_all_teams(self.user_id)
...
class TeamPanel(discord.ui.View):
    def __init__(self, user_id, user_team):
        ...
        self.add_item(TeamInfoButtonModal())
    ...
    class JoinTeamButton(discord.ui.Button):
        def __init__(self):
            super().__init__(label="🗕️ Join Team", style=discord.ButtonStyle.blurple)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if not code.isdigit() or len(code) != 4:
            return await interaction.response.send_message(
                "❌ Code must be a 4-digit number.", ephemeral=True
            )

        if name in teams:
            return await interaction.response.send_message(
                "❌ Team already exists.", ephemeral=True
            )
